...
async function main() {
	...
	const domain = 'https://briantakita.com'
	await Promise.all([
		write__posts__page(),
		write__post__page(),
	...
	async function write__post__page() {
		const htmls = []
		const promises__mkdir = []
		for (let i = 0; i < posts.length; i++) {
			const post = posts[i]
			const { html } = post__page.render(post)
			const build__dir = $build__dir(post)
			htmls.push(html)
	...
	async function $posts() {
		const posts = []
		const paths__source = await promisify(glob)('posts/*.md')
			, promises__markdowns = []
